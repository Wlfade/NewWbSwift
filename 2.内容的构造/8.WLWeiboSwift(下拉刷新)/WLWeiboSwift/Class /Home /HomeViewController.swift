...
extension HomeViewController{
    ...
    private func loadStatus(isNewData:Bool){
        ...
        var since_id = "0"
        if isNewData {
            since_id = viewModelsArr.first?.status?.mid ?? "0"
            
            
            print(viewModelsArr.first?.status?.text as Any)
            print(viewModelsArr.first?.status?.mid as Any)

        }
        
        NetworkTools.shareInstance.loadStatus(since_id: since_id) { (result, error) in
            //1.错误校验
            if error != nil{
                print(error!)
                return
