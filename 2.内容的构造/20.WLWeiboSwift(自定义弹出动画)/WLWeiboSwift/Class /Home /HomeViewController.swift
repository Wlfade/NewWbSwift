...
//MARK:- 图片点击
extension HomeViewController{
    private func registerPhotoBrowerNot(){
        NotificationCenter.default.addObserver(self, selector: #selector(showPhotoBrower(_ :)), name: NSNotification.Name(rawValue: showPhotoBrowserNote), object: nil)
    }
    @objc private func showPhotoBrower(_ notification:Notification){
        print(notification.userInfo as! [String : Any])
        
        //取出数据
        let indexPath = notification.userInfo![showPhotoBrowserIndexKey] as! NSIndexPath
        
        let picUrls = notification.userInfo![showPhotoBrowserUrlsKey] as! [NSURL]
        
        let object = notification.object as! PicCollectionView
        ...
        //2.以modal样式弹出控制器
        self.present(photoBrowerVC, animated: true, completion: nil)
    }
}
//MARK:- 事件监听函数
extension HomeViewController{
